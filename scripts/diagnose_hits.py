...
def _read_file(
    ...
    words = [struct.unpack_from("<Q", raw, i * _WORD)[0] for i in range(n_words)]
...
def _print_axis(
    ...
    print(
        f"      fold sym (k,9-k) : {sym_str}  mean={sym_mean_val:.2f}  "
...
def diagnose(
    ...
    for path in paths:
        ...
        if n_cols != n_cols_ref:
            print(
                f"  WARNING: {path.name} has {n_cols} planes "
                f"(expected {n_cols_ref}) — skipped"
            )
            continue
        for col in range(n_cols):
            _accumulate(words, n_groups, n_cols, col, bit_freqs[col], pop_hists[col])
            if co_counts is not None:
                _accumulate_cofiring(words, n_groups, n_cols, col, co_counts[col])
    ...
    print(f"Source  : {src}")
    print(
        f"Files   : {len(paths)}  |  Total groups: {total_groups}"
...
def main() -> None:
    ...
    p.add_argument(
        "path", type=Path, help="*.bin file or directory containing *.bin files"
    )
    p.add_argument(
        "--max-groups",
        type=int,
    ...
    if args.path.is_dir():
        paths = sorted(f for f in args.path.glob("*.bin") if "_GPS" not in f.name)
        if not paths:
