...
def fit_telescope_alignment(
    ...
    A    = np.column_stack([np.ones(3), z])  # (3, 2)
    Ainv = np.linalg.pinv(A)                 # (2, 3)
    x_fit3 = (A @ (Ainv @ x.T)).T           # (N, 3) predicted by 3-plane fit
    y_fit3 = (A @ (Ainv @ y.T)).T
    # res3_x = x - x_fit3  (diagnostic (a) — available for logging / future use)
    # res3_y = y - y_fit3

    # ── (b) Two-plane prediction (primary output) ──────────────────────
    ...
    for k, (j1, j2) in enumerate(_OTHERS):
        ...
        planes.append(PlaneCorrection(dx, dy, rotation_z))
...
class AlignmentAccumulator:
    ...
    def add(self, hits: list[Hit]) -> AlignmentCorrection | None:
        ...
        silently dropped.  Returns a new AlignmentCorrection when the
        buffer reaches flush_every events; otherwise None.
        """
        if len(hits) != 3:
            return None
